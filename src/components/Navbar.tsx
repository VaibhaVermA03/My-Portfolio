...
const Navbar = () => {
  ...
  const SplitLink = ({ href, text }: { href: string; text: string }) => {
    return (
      <a href={href} className="relative group block cursor-pointer py-1 overflow-visible">
        ...
        {/* 1. Invisible Spacer */}
        <span className="invisible font-body uppercase text-sm tracking-widest font-medium">
          {text}
        </span>

        {/* 2. Top Half (Visible) */}
        <span 
          className="absolute top-0 left-0 text-white/80 font-body uppercase text-sm tracking-widest font-medium transition-transform duration-300 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:-translate-y-[3px]"
  ...
  return (
    <nav 
      className={`fixed top-0 left-0 w-full px-8 py-6 flex justify-between items-center z-50 transition-all duration-300 border-b
      ${scrolled 
    ...
    >
      ...
      <div className="flex items-center gap-4">
        <a href="#" className="cursor-pointer group">
          <h1 className="font-hand text-xl font-bold tracking-wide text-white drop-shadow-sm group-hover:text-gray-300 transition-colors">
            Vaibhav Verma
          </h1>
        </a>
