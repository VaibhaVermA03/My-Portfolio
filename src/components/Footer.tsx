...
const Footer = () => {
  ...
  return (
    <footer id='contact' className="relative z-0 w-full py-20 px-8 md:px-20 overflow-hidden text-white flex flex-col justify-between">
      ...
      {/* Main Content Row */}
      <div className="relative z-10 flex flex-col md:flex-row items-start justify-between gap-12 mb-24 mt-4">
        ...
        >
          <h2 className="font-heading text-8xl md:text-[11rem] lg:text-[13rem] leading-[0.8] mb-2">
            LET'S
          </h2>
          <h2 className="font-heading text-8xl md:text-[11rem] lg:text-[13rem] leading-[0.8] text-transparent stroke-text">
            CONNECT
          </h2>
        </motion.div>

        {/* Right Side: Text & Button Animation (Delayed) */}
        <motion.div 
          initial={{ opacity: 0, y: 50 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.3 }}
          transition={{ duration: 0.8, delay: 0.2, ease: "easeOut" }}
          className="w-full md:w-1/2 md:pl-10 mt-4 md:mt-8"
        >
          
          <p className="font-hand text-3xl md:text-5xl text-gray-300 leading-relaxed">
            ...
            <motion.a 
              ...
              target="_blank"
              // 'align-bottom' ensures it sits perfectly on the line with text
              className="relative inline-block overflow-hidden cursor-pointer group align-bottom"
              initial="initial"
              whileHover="hover"
            >
               {/* 1. Static Text (Space Holder) */}
               <motion.span 
               ...
               >
                  ...
                  >
                     {/* Text Set 1 */}
                     <span className="font-hand font-bold text-3xl md:text-5xl text-orange-400 pr-3">
                        Let's Talk.
                     </span>
                     
                     {/* Text Set 2 (Duplicate) */}
                     <span className="font-hand font-bold text-3xl md:text-5xl text-orange-400 pr-3">
                        Let's Talk.
                     </span>
                  </motion.span>
      ...
      {/* COPYRIGHT & LINKS SECTION (Fade In) */}
      <motion.div 
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
        transition={{ duration: 1, delay: 0.5 }}
        className="relative z-10 w-full border-t border-white/20 pt-8 flex flex-col-reverse gap-6 md:flex-row justify-between items-center text-sm font-body uppercase tracking-wider text-gray-400"
      >
        
        <p>© 2026 Vaibhav Verma. All Rights Reserved.</p>
